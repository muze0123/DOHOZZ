...
watch(() => props.cooperation, (c) => {
  if (c) {
    influencerId.value = c.influencer?.id || ''
    mode.value = c.mode || ''
    deliveryStartTime.value = c.deliveryStartTime || ''
    deliveryEndTime.value = c.deliveryEndTime || ''
    bdId.value = c.bd || ''
    shopId.value = c.shop || ''
    productIds.value = c.products?.map(p => p.id) || []
    slotFee.value = c.slotFee || ''
    note.value = c.note || ''
  }
}, { immediate: true })
